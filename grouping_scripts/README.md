...
- [Future Work & Next Steps](#future-work--next-steps)
  - [The Strategic Entity Prioritization Model](#the-strategic-entity-prioritization-model)
  - [Synthesis & Narrative Generation (LLM Synthesis)](#synthesis--narrative-generation-llm-synthesis)
  - [Architectural Scalability](#architectural-scalability)
  - [Zooniverse & Human-in-the-Loop Validation](#zooniverse--human-in-the-loop-validation)
...
│   ├── test_global.py
│   ├── test_local_predictions.html
│   └── test_global_audit.html
├── output/
│   ├── GLOBAL_MASTER_EXTRACTED.csv
│   ├── MASTER_LOCAL_NAMES.csv
│   ├── LOCAL_LOCATIONS_CLUSTERS.csv
│   ├── LOCAL_DATES_CLUSTERS.csv
│   ├── LOCAL_FILE_CLUSTERS.csv
│   └── linkage_scores_audit.csv
├── utils/
│   ├── clean_utils.py
│   └── db_utils.py
└── run_pipeline.py
...
*   **`output/`**: The destination folder where all final CSV master lists are exported. (Note: HTML audit reports are stored within their respective domain folders).
...
| `LOCAL_FILE_CLUSTERS.csv` | Names (Audit) | Full persons table schema with `cluster_id` for local audit tracking |
| `linkage_scores_audit.csv` | Names (Audit) | Raw Splink match probabilities and weights for every global pairwise comparison |
...
*   **Visual Traceability:** It generates `locations/test_local_predictions.html`. This report lists every resolved location pair, their match probability, and exactly how the city/county/state metadata influenced the final score. This allows a data engineer to explicitly see how a token-overlap rule interacts with clashing metadata, facilitating precise calibration of the final 0.70 deployment threshold.
...
*   **Visual Audit Trail:** It generates `dates/test_local_predictions.html`, a dashboard that tracks the exact number of partial dates forced into singletons and highlights the "Top 50 Largest Exact Date Clusters." This allows data engineers to instantly spot OCR anomalies (e.g., if a file incorrectly extracted "January 1, 1900" 400 times due to a bad bureaucratic stamp).
...
While this pipeline successfully transitions raw OCR text into structured, relational entities, it represents the foundational data layer. Future development should be guided by a strategic hierarchy of accuracy and importance.

### The Strategic Entity Prioritization Model

| Entity Type | Priority | Strategic Rationale |
| :--- | :--- | :--- |
| **Names** | **Highest** | The entire social network and narrative structure depend on correctly identifying human actors. Requires the highest linkage accuracy. |
| **Locations** | **Medium** | Vital for geographical context. Accuracy can be improved by expanding historical abbreviation dictionaries and suffix normalization during initial parsing. |
| **Dates** | **Lowest** | Highly prone to false linkages due to high frequency of shared dates. Dates provide timeline structure but require human-in-the-loop validation for final narratives. |

---

### Synthesis & Narrative Generation (LLM Synthesis)

The ultimate goal of this pipeline is to transform structured data into a cohesive biographical story.

#### 1. Narrative Storytelling
By combining the fully resolved Local Dates, Local Locations, and Global Names, we can construct structured, chronological timelines. These unified event datasets can be fed back into Gemini with storytelling prompts to synthesize fluid, chronologically accurate historical narratives of the USCT soldiers' lives.

#### 2. Event-Centric Knowledge Graphs
Once synthesis is complete, the data should be visualized as an event-centric knowledge graph rather than a simple social network. This graph will capture:
*   **Relationships:** Parent–child, spouse, military camaraderie.
*   **Events:** Enlistment, discharge, marriage, medical evaluations, death.
*   **Spatio-Temporal Context:** The precise time and location associated with every life event.

#### Tooling Recommendations

**Neo4j + Neo4j Bloom — Primary Visualization Layer**
Enables intuitive, large-scale exploration of people, relationships, and events. Ideal for non-technical users who need to query and navigate complex historical networks.

**Dash Cytoscape — Interactive Web Applications**
Best for building custom, user-facing interfaces. Supports features like:
*   Searchable individuals
*   Click-to-expand family trees
*   Integration with timelines and maps

**PyVis — Lightweight Prototyping**
Quickly generates interactive HTML network visualizations from Python. Useful for debugging or sharing small subsets (e.g., a single family or regiment).

**Splink cluster_studio_dashboard — Debugging & Validation**
Built directly into the entity resolution pipeline. Visualizes why records were linked, exposing the underlying matching logic and scores.

**✗ Matplotlib / Seaborn — Not Recommended**
Static visualizations are not suitable for multi-dimensional, relational graph exploration. They lack interactivity and do not scale to this use case.

---

### Architectural Scalability

To transition from a pilot to a production-scale archive, two architectural shifts are recommended:

#### 1. Comprehensive & Constrained AI Extraction
The Phase 2 RAG pipeline should be scaled to finalize extraction for all resolved names across the archive. Furthermore, extraction prompts must be "hardened": specifically, the **Title** field for Names (e.g., "Clerk", "Widow") should be constrained via the LLM prompt to map strictly to a predefined, categorical list. This prevents hallucination and standardizes roles for more accurate linkage.

#### 2. Graph-Driven Sequential Processing (The "360-Degree" View)
Implementation of an intelligent, graph-driven processing queue. If the pipeline extracts "Moses Green" as a witness inside "Wilkins Abbs's" file, the orchestrator should dynamically prioritize Moses Green's individual pension file for the next batch. This recursive approach builds a "360-degree" view of connected individuals, resolving social clusters exponentially faster.

---

### Zooniverse & Human-in-the-Loop Validation

The pipeline generates highly confident clusters (accessed via cluster_ids/global_cluster_ids and csv files), but historical research requires absolute verification. The final phase involves feeding these grouped entities back into **Zooniverse**. By presenting human volunteers with pre-clustered sets of transcriptions, we can:
1.  **Correct OCR Artifacts:** Fix spellings that were too corrupted for automated resolution.
2.  **Validate Links:** Allow historians to confirm or reject borderline probabilistic matches.
3.  **Enrich Narratives:** Add nuanced human observations that LLMs might overlook.
